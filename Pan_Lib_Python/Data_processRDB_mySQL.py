...
def deleteData(dbName="testdb", tableName="winner", sqlStr=""):
    try:
        if dbConn!="":
            myCur = dbConn.cursor()
            if sqlStr == "":
...
def updateData(dbName="testdb", tableName="winner", sqlStr=""):
    try:
        if dbConn!="":
            myCur = dbConn.cursor()
            if sqlStr == "":
                sqlStr = "UPDATE " + tableName + " Set Year=2022"
            myCur.execute(sqlStr)
            dbConn.commit()
            print("=====", myCur.rowcount, "records(s) updated")
        else:
            print("DB connection error, processing abort!!")
    except Exception as err:
        print("Insert Data Table Fail -->", err)
...
###Query
def queryDB(dbConn="", sqlStr=""):
    try:
        if dbConn!="":
            myCur = dbConn.cursor()
            if sqlStr == "":
                sqlStr = "SELECT * FROM winner"
                myCur.execute(sqlStr)
                result = myCur.fetchall()
            #show result
            print("=====", myCur.rowcount, "records(s) quired")
            for rr in result: print(rr)
        else:
            print("DB connection error, processing abort!!")
    except Exception as err:
        print("Query Data Table Fail -->", err) 
        
###pandas
def pandasReadData(myHost="localhost", port="3306", account="root", pw="", dbName="a108222040nobelWinner",
                    tableName="winner2021"):
    try:
        connStr = "mysql://" + account + ":@" + myHost + ":3306/" + dbName + "?charset=utf8mb4&binary_prefix=true"
        ...
        print("Pandas weite fail -->", err) 
    
def pandasWriteData(myHost="localhost", port="3306", account="root", pw="", dbName="a108222040nobelWinner",
                    tableName="winner2021", sourceJson=InputPath+"/Nobel_winner_2021.json"):
    try:
        df = pd.read_json(sourceJson)
        
        connStr = "mysql://" + account + ":@" + myHost + ":" + port + "/" + dbName + "?charset=utf8mb4&binary_prefix=true"
        
        dbConn = sqlalchemy.create_engine(connStr)
...
if (__name__ == "__main__"):
    ...
    createTable(dbConn="", dbName="a108222040nobelWinner", tableName="winner",
                sqlStr="",
                columnDT="Category VARCHAR(30), Name VARCHAR(100), " + \
                          "Nationality VARCHAR(100), Sex VARCHAR(50), Year INT(4)")
    ...
    insertData(dbConn, dbName="a108222040nobelWinner", tableName="winner",
                sqlStr="",
                columnValue="Category, Name, Nationality, Sex, Year",
                value=('test', 'test', 'test', 'test', 2020))
    insertData(dbConn, dbName="a108222040nobelWinner", tableName="winner",
                sqlStr="",
                columnValue="Category, Name, Nationality, Sex, Year",
                value=('test2', 'test2', 'test2', 'test2', 2020))
    updateData(dbConn, tableName="winner" ,sqlStr="UPDATE winner SET Year=2022 WHERE Year < 2021 ")
    deleteData(dbConn, sqlStr="DELETE FROM winner WHERE Year=2022")
    # deleteData(dbConn, sqlStr="SELECT Sex, Year FROM winner WHERE Year=2022")
    queryDB(dbConn, sqlStr="")
    #queryDB(dbConn, sqlStr="SELECT Sex, Year FROM winner WHERE Year=2020")
    pandasWriteData(myHost="localhost", port="3306", account="root", pw="", dbName="a108222040nobelWinner",
                      tableName="winner2021", sourceJson=InputPath+"/Nobel_winner_2021.json")
    pandasReadData(myHost="localhost", port="3306", account="root", pw="", dbName="a108222040nobelWinner",
                      tableName="winner2021")
    dbConn.close()